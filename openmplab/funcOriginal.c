...
void func0(double *weights, double *arrayX, double *arrayY, int xr, int yr, int n)
{
	int i;
	for(i = 0; i < n; i++){
	  weights[i] = 1/((double)(n));    // 1/double(n) can be taken out of loop
	  arrayX[i] = xr;                  //
	  arrayY[i] = yr;                  // 
	}
}
			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	...
	int max_size = X*Y*Z;
   	...
   	for(i = 0; i < n; i++){
	  arrayX[i] += 1 + 5*rand2(seed, i);  //call rand2() only once for both this line and below line
	  arrayY[i] += -2 + 2*rand2(seed, i); //
   	...
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
   			...
   			index[i*Ones + j] = fabs(index_X*Y*Z + index_Y*Z + iter); // dereferencing index[i*Ones + j] this and next line and below loop
   			if(index[i*Ones + j] >= max_size)
   				index[i*Ones + j] = 0;
   		}
   		probability[i] = 0; // dereference probability[i] only once per loop iteration here + below 2
...
void func3(double *arrayX, double *arrayY, double *weights, double *x_e, double *y_e, int n)
{
	...
	for(i = 0; i < n; i++){
	  estimate_x += arrayX[i] * weights[i]; // dereference weights[i]
...
void func4(double *u, double u1, int n)
{
	...
	for(i = 0; i < n; i++){
	  u[i] = u1 + i/((double)(n)); // take casting of n out
   	}
}
...
void func5(double *x_j, double *y_j, double *arrayX, double *arrayY, double *weights, double *cfd, double *u, int n)
{
	...
	for(i = 0; i < n; i++){
	  arrayX[i] = x_j[i]; // setting value kind of pointless?
	  arrayY[i] = y_j[i]; 
